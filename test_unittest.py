...
class TestApplication(unittest.TestCase):
    ...
    def test_get_request_complex(self):
        ...
        job_list = ScheduleFlow.Application(10, 0, 200, [100, 200, 300])
        self.assertEqual(job_list.get_request_time(4), -1)
    ...
    def test_invalid_factor(self):
        with self.assertRaises(AssertionError):
            ScheduleFlow.Application(7, 5, 10, [11],
                                     resubmit_factor=-2)
        # valid factor at creation but not at update
        job = ScheduleFlow.Application(7, 5, 10, [9],
                                       resubmit_factor=1)
        with self.assertRaises(AssertionError):
...
class TestCheckpointing(unittest.TestCase):
    ...
    def test_get_checkpoint(self):
        job = ScheduleFlow.Application(10, 0, 200, [100, 200, 300],
                                       resubmit_factor=1.5)
        job.set_checkpointing([25])
        self.assertEqual(job.get_checkpoint_size(100), 25)
        job.set_checkpointing([25, 20, 10, 50])
    ...
    def test_get_current_request(self):
        ...
        # check that all submissions read the last checkpoint
        system = ScheduleFlow.System(10, io_read_bw=2)
        job = ScheduleFlow.Application(10, 0, 200, [50, 80, 130])
        job.set_checkpointing([10, -1])
        job.assign_system(system)
        job.update_submission(60)
        job.update_submission(145)
        self.assertEqual(job.get_current_total_request_time(), 135)

    def test_nofit_in_schedule(self):
        system = ScheduleFlow.System(10, io_read_bw=2)
        sch = ScheduleFlow.Scheduler(system)
        reservation = {}
        for i in range(5):
            for _ in range(2):
    ...
    def test_simulation_correctness(self):
        ...
        job_list = [
                    ScheduleFlow.Application(5, 0, 500, [200],
                                             resubmit_factor=1.5),
                    ScheduleFlow.Application(5, 0, 500, [100, 200, 300, 400]),
                    ScheduleFlow.Application(5, 0, 500, [300, 100]),
                    ScheduleFlow.Application(5, 0, 400, [200, 300, 200]),
                    ScheduleFlow.Application(5, 0, 350, [200, 100, 200],
                                             resubmit_factor=1.5),
                    ScheduleFlow.Application(5, 0, 700, [200],
                                             resubmit_factor=1)
                    ]
        job_list[0].set_checkpointing([100])
        job_list[1].set_checkpointing([100, -1, 10])
        job_list[2].set_checkpointing([100])
        job_list[3].set_checkpointing([-1, 100, -1])
        job_list[4].set_checkpointing([100, -1])
        job_list[5].set_checkpointing([50])
        sim.create_scenario(
            ScheduleFlow.Scheduler(ScheduleFlow.System(10)),
            job_list=job_list)
        sim.run()
...
class TestScheduleGaps(unittest.TestCase):
    ...
    def test_left_void_in_middle(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(10), 10)
        reservation = {}
        reservation[ScheduleFlow.Application(5, 0, 50, [50])] = 0
        reservation[ScheduleFlow.Application(5, 0, 50, [50])] = 100
        reservation[ScheduleFlow.Application(10, 0, 50, [50])] = 150
        gaps = sch.gaps_in_schedule.add(reservation)
        self.assertTrue([50, 100, 10] in gaps)
        self.assertSequenceEqual([i[0] for i in gaps], [0, 50])

    def test_right_void_in_middle(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(10), 10)
        reservation = {}
        reservation[ScheduleFlow.Application(10, 0, 50, [50])] = 0
        reservation[ScheduleFlow.Application(5, 0, 50, [50])] = 100
        reservation[ScheduleFlow.Application(5, 0, 50, [50])] = 150
        gaps = sch.gaps_in_schedule.add(reservation)
        self.assertTrue([50, 100, 10] in gaps)
        self.assertTrue(all(x[0] == 50 for x in gaps))
    ...
    def test_remove_middle_jobs(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(10))
        job = ScheduleFlow.Application(1, 0, 2, [5])
        reservations = {}
        reservations[ScheduleFlow.Application(5, 0, 2, [5])] = 0
        reservations[ScheduleFlow.Application(3, 0, 1, [2])] = 3
        reservations[ScheduleFlow.Application(9, 0, 3, [5])] = 5
    ...
    def test_trim(self):
        ...
        reservations[ScheduleFlow.Application(10, 0, 2, [5])] = 15
        gaps = sch.gaps_in_schedule.add(reservations)
        gaps = sch.gaps_in_schedule.trim(4)
...
class TestScheduler(unittest.TestCase):
    def test_nofit_end(self):
        ...
        sch = ScheduleFlow.Scheduler(system, 15)
        reservation = {}
        for i in range(5):
    ...
    def test_fit_end(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(10), 15)
        reservation = {}
        for i in range(5):
            reservation[ScheduleFlow.Application(5, 0, 50, [50])] = i * 50
            reservation[ScheduleFlow.Application(5, 0, 50, [50])] = i * 50
    ...
    def test_one_node_jobs(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(100), 105)
        np.random.seed(0)
    ...
    def test_system_wide_jobs(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(10), 105)
        np.random.seed(0)
        num_jobs = np.random.randint(1, 101)
        for i in range(num_jobs):
            exect = np.random.randint(10, 100)
...
class TestRuntime(unittest.TestCase):
    ...
    def test_nofail_random(self):
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(100))
        job_list = set()
        np.random.seed(0)
        for i in range(100):
            execution_time = np.random.randint(11, 100)
    ...
    def test_backfill_nojob(self):
        for SchedulerType in ScheduleFlow.Scheduler.__subclasses__():
            ...
            cnt = 0
            if SchedulerType == ScheduleFlow.Scheduler:
                cnt = 1
            self.assertEqual(
    ...
    def test_backfill_jobs(self):
        ...
        sch = ScheduleFlow.Scheduler(ScheduleFlow.System(10), 9)
        job_list = set()
        for i in range(5):
            time = 50
            if i == 3:
    ...
    def test_backfill_gaps(self):
        ...
        for i in range(5):
            if i == 2:
                continue
            job_list.add(ScheduleFlow.Application(5, 100, 50, [50]))
            job_list.add(ScheduleFlow.Application(5, 100, 50, [50]))
        job_list.add(ScheduleFlow.Application(5, 100, 50, [60]))
        job_list.add(ScheduleFlow.Application(5, 100, 50, [70]))
        job_list.add(ScheduleFlow.Application(10, 100, 10, [10]))
        runtime = Runtime(job_list)
        runtime(sch)
        workload = runtime.get_stats()
        # check that all jobs are executed between 100 and 370
        self.assertTrue(max([workload[job][len(workload[job]) - 1][1]
                             for job in workload]) <= 370)
        self.assertTrue(min([workload[job][len(workload[job]) - 1][1]
                             for job in workload]) >= 100)
